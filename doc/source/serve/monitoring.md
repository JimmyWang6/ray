...
| `ray_serve_deployment_max_queued_queries` **[H]** | Gauge | `deployment`, `application`, `handle`, `actor_id` | Configured maximum number of requests that can be queued. -1 indicates no limit. Helps contextualize queue saturation. |
...
| `ray_serve_deployment_max_ongoing_requests` **[H]** | Gauge | `deployment`, `application`, `handle`, `actor_id` | Configured maximum number of ongoing requests allowed per replica. Helps contextualize replica saturation. |
